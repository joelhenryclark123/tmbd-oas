...
module OpenapiClient
  class AccountRatedTv200ResponseResultsInner
    attr_accessor :adult
    ...
    def self.openapi_types
      {
        ...
        :'vote_average' => :'Float',
        :'vote_count' => :'Integer',
        :'account_rating' => :'AccountRatedTv200ResponseResultsInnerAccountRating'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::AccountRatedTv200ResponseResultsInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::AccountRatedTv200ResponseResultsInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'origin_country')
        if (value = attributes[:'origin_country']).is_a?(Array)
    ...
    def hash
      [adult, backdrop_path, genre_ids, id, origin_country, original_language, original_name, overview, popularity, poster_path, first_air_date, name, vote_average, vote_count, account_rating].hash
    end
